...
import com.cgi.hexagon.kafka.messages.Producer;
...
@Configuration
@Import({BonnieRestConfiguration.class, H2StorageApplication.class, Producer.class})
public class BonnieConfiguration {

    @Bean
    OrderService orderService(OrderStorage orderStorage, UserStorage userStorage, Producer producer) {
        return new OrderService(orderStorage, userStorage, producer);
    }
